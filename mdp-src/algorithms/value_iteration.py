...
def is_terminal_state(board, i, j) -> bool:
    return board[i][j] != -1


def check_policy_convergence(V_k: List[List[float]], V_k_next: List[List[float]], epsilon: float = 1e-6) -> bool:
...
def value_iteration(board: List[List[int]], V_k: List[List[float]], gamma=0.99, k=0) -> Tuple[List[List[float]], List[List[str]]]:
        ...
        for j in range(num_columns):
            ...
            for action in ['N', 'W', 'S', 'E']:
                ...
                for successor in get_successors(i, j, action):
                    ...
                    # print(f"sum += {transition} * (board[{i}][{j}] + gamma * V_k[{actX}][{actY}])"
                    #       f" = sum += {transition} * ({board[i][j]} + {gamma} * {V_k[actX][actY]})")
                    # print("sum = ", sum)
                    first_successor = False
                if sum > max_next_V:
                    max_next_V = sum
                    max_arg_s = action
            # print(f"max sum for state = board[{i}][{j}] is {round(max_next_V, 2)} and action is '{max_arg_s}'")
            V_k_next[i][j] = round(max_next_V, 2)
    ...
    if check_policy_convergence(V_k, V_k_next):
